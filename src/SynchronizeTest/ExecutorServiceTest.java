 ...
 * (2)execute 无法返回线程执行结果
 * 	  submit 可以接受Future<String> 执行结果
 * (3)execute无法处理异常
 * 	  submit 在call()方法中抛出异常。在Future.get中捕获异常，进行处理
 * @author zhoushaojun
 *
 */
...
class callableTask implements Callable<String> {
	...
	public String call() throws Exception {
		...
		return "lengjing + call()方法被自动调用，任务的结果是：" + id + "    " + Thread.currentThread().getName();
	}
